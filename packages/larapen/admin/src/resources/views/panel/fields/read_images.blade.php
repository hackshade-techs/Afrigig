...
<div @include('admin::panel.inc.field_wrapper_attributes') >
	...
	@if (!empty($entity_model) && !$entity_model->isEmpty())
		...
		@endforeach
        @if ($entity_model->count() < $ads_pictures_number)
            <hr><br>
            <a href="{{ url(config('larapen.admin.route_prefix', 'admin') . '/picture/create?ad_id=' . Request::segment(3)) }}" class="btn btn-xs btn-default"><i class="fa fa-edit"></i> {{ trans('admin::messages.add') }} picture</a>
        @endif
	@else
