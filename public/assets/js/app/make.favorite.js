...
{
    var pElmtId = $(elmt).closest('li').attr('id');
    ...
    $.ajax({
        ...
        if (data.status==1) {
            if ($(elmt).hasClass('btn')) {
                $('#' + data.adId).removeClass('saved-job').addClass('saved-job');
                $('#' + data.adId + ' a').removeClass('save-job').addClass('saved-job');
            } else {
                $(elmt).html('<span class="fa fa-star"></span> Saved Job');
            }
        } else {
            if ($(elmt).hasClass('btn')) {
                $('#' + data.adId).removeClass('save-job').addClass('save-job');
                $('#' + data.adId + ' a').removeClass('saved-job').addClass('save-job');
            } else {
                $(elmt).html('<span class="fa fa-star-o"></span> Save Job');
            }
        }
        return false;
    });
