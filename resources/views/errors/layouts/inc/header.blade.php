				...
				<div class="navbar-collapse collapse">
					...
					<ul class="nav navbar-nav navbar-right">
						@if (!auth()->user())
							...
							<li class="postadd">
								<a class="btn btn-block btn-post btn-yellow" href="{{ url(trans('routes.create')) }}"> {{ t('Create a Job ad') }}</a>
							</li>
						@else
							@if (isset($user))
                                ...
                                @if (in_array($user->user_type_id, [1, 2]))
								<li class="postadd">
									<a class="btn btn-block btn-post btn-yellow" href="{{ url(config('app.locale') . '/' . trans('routes.create')) }}"> {{ t('Create a Job ad') }}</a>
								</li>
                                @endif
							@endif
						@endif
						...
						@if (isset($lang))
							@if (count(LaravelLocalization::getSupportedLocales()) > 1)
								<!-- Language selector -->
								<div class="dropdown hidden-sm hidden-xs" style="float:right; margin: 2px 0 0 5px;">
